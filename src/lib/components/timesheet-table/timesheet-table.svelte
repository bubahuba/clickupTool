<script lang="ts">
  ...
  interface Props {
    ...
    currentUser?: {
      ...
      initials?: string;
      color?: string;
    }; // fallback when timesheets empty
    users?: TimesheetUser[];
  ...
  const shouldFetchPrediction = $derived.by(() => {
    ...
    return (
      lastDay.getTime() >= todayStartMs && firstDay.getTime() <= predictionEndMs
    );
  });
  ...
  const taskSlots = $derived(
    buildTaskSlotsFromEstimatedTasks(estimatedTasks, MAX_HOURS_DEFAULT),
  );
  ...
  const displayTimesheets = $derived.by((): UserTimesheet[] => {
    const hasCurrentUser =
      currentUserId != null &&
      usersTimesheets.some(
        (userTimesheet) => userTimesheet.user.id === currentUserId,
      );
...
<Tooltip.Provider skipDelayDuration={3000}>
  <div class={cn("flex flex-col gap-4", className)}>
    <div class="flex items-center justify-between gap-4 flex-wrap">
      <div
        class="flex items-center gap-4 text-xs text-muted-foreground shrink-0 mr-4"
      >
        <span class="flex items-center gap-1.5">
          <span class="w-2 h-2 rounded-[2px] shrink-0 bg-[hsl(217_91%_60%)]"
          ></span>
          {m.timesheet_legend_tracked()}
        </span>
        <span class="flex items-center gap-1.5">
          <span class="w-2 h-2 rounded-[2px] shrink-0 bg-[hsl(25_95%_53%)]"
          ></span>
          {m.timesheet_legend_over()}
        </span>
        <span class="flex items-center gap-1.5">
          <span class="w-2 h-2 rounded-[2px] shrink-0 bg-(--capacity-pred-2)"
          ></span>
      ...
      <div class="flex items-center gap-4">
        <Button
          ...
          size="icon"
          onclick={prevMonth}
          aria-label={m.aria_previous_month()}
        >
          <ChevronLeft class="size-4" />
        </Button>
        <span
          class="font-semibold min-w-40 text-center"
          style="font-family: var(--font-heading)">{monthLabel}</span
        >
        <Button
          variant="outline"
          size="icon"
          onclick={nextMonth}
          aria-label={m.aria_next_month()}
        >
    ...
    <div class="overflow-x-auto border border-border rounded-lg pb-10">
      <table
        class="w-max min-w-full table-fixed border-separate border-spacing-0"
      >
        <TableHeader>
          <TableRow>
            <TableHead
              class="sticky left-0 z-3 bg-background min-w-16 lg:min-w-40 whitespace-nowrap shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] text-[0.65rem] text-muted-foreground"
              >{m.user()}</TableHead
            >
            {#each daysInMonth as dayItem, _dayIndex (dayItem.key)}
              <TableHead
                class={cn(
                  "min-w-8 text-center p-1 align-middle text-[0.65rem] text-muted-foreground",
                  dayItem.isWeekend && "bg-muted",
                )}
              >
        ...
        <TableBody>
          {#each displayTimesheets as { user, hoursByDay }, _userIndex (user.id)}
            <TableRow>
              <TableCell
                class="sticky left-0 z-2 bg-background min-w-16 lg:min-w-40 whitespace-nowrap shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] font-medium"
              >
                <span
                ...
                >
                  <span
                    class="inline-flex items-center justify-center w-7 h-7 rounded-md bg-(--user-color) text-white text-xs font-semibold shrink-0"
                    >{getInitials(user.username, user.initials)}</span
                  >
                  <span class="overflow-hidden text-ellipsis hidden lg:block"
                    >{user.username}</span
                  >
                </span>
              </TableCell>
              {#each daysInMonth as dayItem, _dayIndex (dayItem.key)}
                {@const effective = getEffectiveDayDetails(
                  hoursByDay,
                  dayItem.key,
                  dayItem.isFuture,
                  user.id,
                ...
                <Tooltip.Root>
                  <TableCell
                    class={cn(
                      "min-w-8 h-10 p-0 align-bottom text-center relative hover:z-1",
                      dayItem.isWeekend && "bg-muted",
                    )}
                  >
                    <Tooltip.Trigger class="group w-full h-full">
                      <span
                        class="hidden group-hover:block absolute -top-4 -left-4 -right-4 -bottom-4"
                      ></span>
                      <div
                        class={cn(
                        ...
                        style="--bar-height: {barHeight}%; transition-delay: {Math.random() *
                          1000}ms;"
                      ...
                      >
                        {#if dayDetails?.tasks?.length}
                          <div class="min-w-48 space-y-1.5 py-0.5">
                            <div
                              class="font-medium text-[0.65rem] text-muted-foreground border-b border-border pb-1 mb-1"
                            >
