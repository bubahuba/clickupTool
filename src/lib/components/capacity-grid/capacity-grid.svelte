<script lang="ts">
  ...
  function handlePredictionToggle(checked: boolean) {
    ...
    goto(
      resolve((url.pathname + url.search) as import("$app/types").Pathname),
      {
        replaceState: true,
      },
    );
  ...
  const gridData = $derived.by(() => {
    ...
    const totalDays =
      Math.ceil(
        (endSunday.getTime() - startMonday.getTime()) / (24 * 60 * 60 * 1000),
      ) + 1;
  ...
  <div class="flex items-center justify-between gap-2">
    <div class="flex items-center gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-2 text-xs">
        <span class="text-muted-foreground">{m.capacity_legend_less()}</span>
        <div class="flex gap-0.5 items-center">
          <span
            class="w-4 h-4 rounded-[2px] shrink-0 {predictionMode
              ? 'bg-(--capacity-pred-empty)'
              : 'bg-(--capacity-cell-empty)'}"
          ></span>
          <span
            class="w-4 h-4 rounded-[2px] shrink-0 {predictionMode
              ? 'bg-(--capacity-pred-1)'
          ...
          <span
            class="w-4 h-4 rounded-[2px] shrink-0 ml-1 border-l border-border pl-1 {predictionMode
              ? 'bg-(--capacity-pred-over)'
              : 'bg-(--capacity-cell-over)'}"
          ></span>
        </div>
        <span class="text-muted-foreground">{m.capacity_legend_more()}</span>
      </div>
    </div>
    ...
    <Tooltip.Provider>
      <div class="flex gap-3 items-stretch min-w-0">
        <div class="flex flex-col items-end shrink-0">
          <div class="h-6.5 shrink-0"></div>
          <div
            class="grid gap-0.5 pr-(--cell-gap,2px)"
          ...
          >
            {#each gridData.rows as row, rowIndex (rowIndex)}
              <span
                class="text-[0.65rem] text-muted-foreground leading-(--cell-size,1rem) flex items-center justify-end"
                >{row.dayLabel}</span
              >
            {/each}
          </div>
        ...
        >
          ...
          >
            {#each gridData.monthLabels as monthLabel, _monthIndex (monthLabel.label + monthLabel.col)}
              <span
                class="text-[0.65rem] text-muted-foreground grid-row-1 leading-(--cell-size,1rem)"
            ...
            {#each gridData.rows as row, rowIndex (rowIndex)}
              {#each row.cells as cell, colIndex (cell.dateKey || `${row.dayLabel}-${colIndex}`)}
                ...
                <div
                  ...
                  class="relative w-full h-full min-w-(--cell-size,1rem) min-h-(--cell-size,1rem) transition-background-color duration-300"
                ...
                >
                  {#if hasData}
                    <Tooltip.Root>
                      <Tooltip.Trigger
                        class="group block w-full h-full absolute inset-0 cursor-default rounded-[2px] transition-[outline] duration-100"
                        aria-label={formatDayCellLabel(cell.date, locale)}
                      >
                        <span
                          class="hidden group-hover:block absolute -top-2 -left-2 -right-2 -bottom-2"
                        ></span>
                      </Tooltip.Trigger>
                      <Tooltip.Content
                      ...
                      >
                        {#if cell.details?.tasks?.length}
                          <div class="min-w-48 space-y-1.5 py-0.5">
                            ...
                            >
                              {formatDayCellLabel(cell.date, locale)}
                  ...
                  {:else}
                    <div
                      class="block w-full h-full rounded-[2px] bg-(--capacity-cell-empty)"
                      aria-hidden="true"
                    ></div>
                  {/if}
                </div>
